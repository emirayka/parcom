import {Parser, ParserErrorFailure, ParserErrorIncomplete, ParserErrorSeparatedNonemptyList} from '@/types'
import {tag} from '@/parser/string/streaming'
import {separatedNonemptyList} from '@/combinator/multi/separated-nonempty-list'
import {Err, Ok} from '@emirayka/option-result'

describe('separatedNonemptyList', () => {
  describe('returns parser that', () => {
    const separator: Parser<string, string> = tag(', ')
    const separated: Parser<string, string> = tag('hello')
    const parser: Parser<string, Array<string>> = separatedNonemptyList(separator, separated)

    const parserThatAlwaysFails: Parser<string, string> = (input: string) => Err([input, new ParserErrorFailure(`${input}${input}`)])
    const parserWithFailedSeparator: Parser<string, Array<string>> = separatedNonemptyList(parserThatAlwaysFails, separated)
    const parserWithFailedSeparated: Parser<string, Array<string>> = separatedNonemptyList(separator, parserThatAlwaysFails)

    test('when there was no match, returns Ok', () => {
      expect(parser('helld')).toEqual(Err(['helld', new ParserErrorSeparatedNonemptyList()]))
    })
