...
import {ParserErrorLineEnding} from '@/types'
...
describe('complete', () => {
  describe('lineEnding', () => {
    ...
    test('when input is empty or equals to \\r, returns Err', () => {
      expect(lineEnding('')).toEqual(Err(['', new ParserErrorLineEnding()]))
      expect(lineEnding('\r')).toEqual(Err(['\r', new ParserErrorLineEnding()]))
    })
  })
})
